...
我理解三者是"大脑—工作记忆—操作手册"的关系：上下文是 Agent 的工作记忆，Skill 是外置的操作手册，Agent 是调度两者完成任务的大脑。三者缺一不可——没有上下文，Agent 不知道任务是什么、也看不到自己刚做了什么；没有 Skill，每换一个新任务都要在上下文里从零交代一遍做法，既浪费窗口又容易走样。

这个作业本身就是三者协作的一次演示：我作为用户把任务写进 AI 助手的上下文，助手（Agent）读取仓库里的 SKILL.md 获得固定的生成流程和自检清单，然后一步步产出学习资料。我由此体会到，上下文解决的是"现在知道什么"，Skill 解决的是"长期怎么做"——把流程沉淀成文件后，AI 的能力就不再依赖某一次对话的临场发挥，这也是我觉得 Skill 最有价值的地方。
## 参考来源
